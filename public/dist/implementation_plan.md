...
- [ ] **Tabel `payments`** (Detail Pembayaran):
  - Kolom: `booking_id` (foreign key), `order_id` (string unik untuk midtrans), `gross_amount` (decimal), `payment_type` (string, misal: 'bank_transfer', 'gopay'), `transaction_status` (string), `snap_token` (string, nullable), `paid_at` (datetime, nullable).
