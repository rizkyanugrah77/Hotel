<div x-data="{
    ...
    x-on:gallery-created.window="showToast($event.detail.message, $event.detail.type); $dispatch('close-modal', 'manage-gallery')"
    x-on:gallery-deleted.window="showToast($event.detail.message, $event.detail.type); $dispatch('close-modal', 'delete-gallery')"
    x-on:gallery-updated.window="showToast($event.detail.message, $event.detail.type); $dispatch('close-modal', 'manage-gallery')"
    ...
    x-on:gallery-delete-confirmation.window="$dispatch('open-modal', 'delete-gallery')">
            ...
            <div class="bg-white border border-gray-100 shadow-sm rounded-2xl p-6">
                <div class="flex justify-between items-start mb-4">
                    <div>
                        ...
                        <h3 class="text-2xl font-poppins font-bold text-accent-600" id="kpiFeatured">0</h3>
                    </div>
            ...
            <div class="bg-white border border-gray-100 shadow-sm rounded-2xl p-6">
                <div class="flex justify-between items-start mb-4">
                    <div>
                        ...
                        <h3 class="text-2xl font-poppins font-bold text-blue-600" id="kpiRooms">0</h3>
                    </div>
            ...
            <div class="bg-white border border-gray-100 shadow-sm rounded-2xl p-6">
                <div class="flex justify-between items-start mb-4">
                    <div>
                        ...
                        <h3 class="text-2xl font-poppins font-bold text-gray-600" id="kpiRegular">0</h3>
                    </div>
                ...
                <div class="flex items-center gap-3">
                    <div class="relative">
                        ...
                        <input type="text" id="searchInput" placeholder="Cari gambar..."
                            class="input pl-10 pr-4 py-2 text-sm w-64" oninput="filterGalleries()" />
                    </div>
                    <select id="filterFeatured" class="input py-2 text-sm w-40" onchange="filterGalleries()">
                        <option value="">Semua Status</option>
                        <option value="true">Featured</option>
                        <option value="false">Non-Featured</option>
                    </select>
                    <select id="filterRoom" class="input py-2 text-sm w-44" onchange="filterGalleries()">
                        <option value="">Semua Room</option>
                    </select>
                    <button @click="$dispatch('open-modal', 'manage-gallery')" class="btn-primary text-sm px-4 py-2">
                            ...
                            <tr>
                                ...
                                <td class="py-3 px-6">
                                    <button wire:click="edit({{ $gallery->id }})"
                                        class="btn-warning text-sm px-4 py-2">
                                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="2"
                                            stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round"
    ...
    <x-modal-2 name="manage-gallery" title="Tambah Gambar" subtitle="Formulir untuk menambahkan gambar baru">
        <form wire:submit="save">
            ...
            <!-- Image Path -->
            <div class="mb-4">
                <label class="input-label">Image Path <span class="text-red-500">*</span></label>
                <input type="file" class="input" wire:model="image" accept="image/*"
                    placeholder="e.g. ../resources/assets/images/gallery-1.jpg" required />
                @if ($image)
